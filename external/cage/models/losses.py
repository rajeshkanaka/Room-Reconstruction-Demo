...
def dn_L1_loss(src_polys, target_polys, target_len,angle=False):
    ...
    Args:
        src_polys: Tensor of dim [num_target_polys, num_queries_per_poly*4] with the matched predicted polygons coordinates
        target_polys: Tensor of dim [num_target_polys, num_queries_per_poly*4] with the target polygons coordinates
        target_len: list of size num_target_polys, each element indicates 4 * num_corners of this poly
    """
    total_loss = 0.
    angle_loss = 0.
    index = 0
    ...
    for length in target_len:
        ...
        if angle:
            min_dis_idx = torch.argmin(all_dis).item()
...
def custom_L1_loss(src_polys, target_polys, target_len,angle=False):
    ...
    for i in range(num_poly):
        ...
        all_polys = get_all_order_corners(tgt_poly_single)
        all_dis = torch.cdist(src_polys[i, :target_len[i]].unsqueeze(0), all_polys, p=1) 
        total_loss += all_dis.min()
        if angle:
            min_dis_idx = torch.argmin(all_dis)
            tgt_matched_poly = all_polys[min_dis_idx].view(-1,4)
