...
def benchmark(args):
    _outputs = {}
    dataset = build_dataset(image_set=args.eval_set, args=args)
    ...
    for name, param in model.named_parameters():
        ...
        print(f"{name:<50}  "
            f"params={param_count:<10} requires_grad={param.requires_grad}")
    ...
    output_file = os.path.join( "flops",f"{args.num_queries}", "log.txt")
